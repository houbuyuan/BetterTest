...
void setpin(GPIO_TypeDef* pinG, uint16_t pinN, BitAction level)
{
	GPIO_WriteBit(pinG, pinN, level);
}
uint8_t readpin(GPIO_TypeDef* pinG, uint16_t pinN)
{
	return GPIO_ReadInputDataBit(pinG, pinN);
}
//硬复位MP3
//返回1:复位失败;0:复位成功
u8 VS_HD_Reset(void)
{
	...
	setpin(VS_RST_G, VS_RST, Bit_RESET);
	delay_ms(20);
	setpin(VS_XDCS_G, VS_XDCS, Bit_SET);//取消数据传输
	setpin(VS_XCS_G, VS_XCS, Bit_SET);//取消数据传输
	setpin(VS_RST_G, VS_RST, Bit_SET);
...
void VS_Sine_Test(void)
{
	...
	VS_SPI_ReadWriteByte(0x00);
	delay_ms(100);
	setpin(VS_XDCS_G, VS_XDCS, Bit_SET);
	//退出正弦测试
	setpin(VS_XDCS_G, VS_XDCS, Bit_RESET);//选中数据传输
...
u16 VS_Ram_Test(void)
{
	...
	while (readpin(VS_DQ_G, VS_DQ)==0); // 等待DREQ为高
	VS_SPI_SpeedLow();//低速
	setpin(VS_XDCS_G, VS_XDCS, Bit_RESET);// xDCS = 1，选择VS10XX的数据接口
...
void VS_WR_Cmd(u8 address,u16 data)
{
	...
	VS_SPI_SpeedLow();//低速
	setpin(VS_XDCS_G, VS_XDCS, Bit_SET);
	setpin(VS_XCS_G, VS_XCS, Bit_RESET);
	...
	setpin(VS_XCS_G, VS_XCS, Bit_SET);
	VS_SPI_SpeedHigh();//高速
}
//向VS10XX写数据
//data:要写入的数据
void VS_WR_Data(u8 data)
{
	...
	setpin(VS_XDCS_G, VS_XDCS, Bit_RESET);
